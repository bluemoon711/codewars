...
Let us call weight of a number the sum of its digits.For example 99 will have "weight" 18, 100 will have
"weight" 1.
...
function closest(strng) {
  ...
  strng = strng.split(' ');
  const weight = s =>
    s.split('').reduce((a, c) => parseInt(a) + parseInt(c), 0);
  const weights = strng
    .map((el, i) => {
      return { e: +el, i: i, w: weight(el) };
    })
    .sort((a, b) => (a.w != b.w ? a.w - b.w : a.i - b.i));
  console.log(weights);

  let diff = weights[weights.length - 1].w - weights[0].w;
  let idx = -1;
  for (let i = 1; i < weights.length; i++) {
    if (weights[i].w - weights[i - 1].w < diff) {
      diff = weights[i].w - weights[i - 1].w;
      idx = i - 1;
    }
  }

  const ret = w => [w.w, w.i, w.e];
  console.log(ret);
  let returnVal = [ret(weights[idx]), ret(weights[idx + 1])];
  return returnVal;
}
